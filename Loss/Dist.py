...
class Dist(nn.Module):      # to compute the distance, such as: d_e(x,y) or d_d(x,y)
    def __init__(self, num_classes=10, num_centers=1, feat_dim=2, init='random'):
        ...
        if init == 'random':
            self.centers = nn.Parameter(torch.randn(num_classes * num_centers, self.feat_dim))
            # self.centers = nn.Parameter(torch.randn(num_classes * num_centers, self.feat_dim))        # P^k
        else:
    ...
    def forward(self, features, center=None, metric='l2'):
        if metric == 'l2':
            f_2 = torch.sum(torch.pow(features, 2), dim=1, keepdim=True)        # [64, 1]
            if center is None:
                ...
                c_2 = torch.sum(torch.pow(center, 2), dim=1, keepdim=True)      # [6, 1]
                dist = f_2 - 2*torch.matmul(features, torch.transpose(center, 1, 0)) + torch.transpose(c_2, 1, 0)
                # print(dist.shape, 'fff')
                # [64, 1] - [64, 6] + [1, 6] 前后两个矩阵会自延拓成中间那个的形状！！！，[64, 6] it's amazing!!!
            dist = dist / float(features.shape[1])
        else:
            if center is None:
                center = self.centers 
            else:               # default compute: d_d(features, P^k)
                center = center 
