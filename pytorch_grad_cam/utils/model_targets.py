...
class FinerWeightedTarget:
    ''' 加权融合主类别与对比类别的梯度信息，优化CAM的定位精度 '''
    def __init__(self, main_category, comparison_categories, alpha):
        self.main_category = main_category  # 主类别索引（如预测结果）
        self.comparison_categories = comparison_categories  # 对比类别索引列表
        self.alpha = alpha  # 对比类别抑制强度
    ...
    def __call__(self, model_output):
        ...
        if len(model_output.shape) == 1:
            ...
            prob = torch.softmax(model_output, dim=-1)  # 模型输出的概率分布
