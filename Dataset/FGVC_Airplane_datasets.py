...
def get_aircraft_datasets(train_transform, test_transform, balance_open_set_eval=False, split_train_val=True, seed=0,
                          **options):

    np.random.seed(seed)

    # Init train dataset and subsample training classes
    train_dataset_whole = FGVCAircraft(root=options['data_root'], transform=train_transform, split='trainval')
    train_dataset_whole = subsample_classes(train_dataset_whole, include_classes=options['known'])
    ...
    test_dataset_known = FGVCAircraft(root=options['data_root'], transform=test_transform, split='test')
    test_dataset_known = subsample_classes(test_dataset_known, include_classes=options['known'])

    options['unknown'] = list(set(range(100)) - set(options['known']))
    # Get testset for unknown classes
    test_dataset_unknown = FGVCAircraft(root=options['data_root'], transform=test_transform, split='test')
    test_dataset_unknown = subsample_classes(test_dataset_unknown, include_classes=options['unknown'])
...
if __name__ == '__main__':
    ...
    options_['data_root'] = 'F:/CV_data/fgvc-aircraft-2013b'

    x = get_aircraft_datasets(None, None, split_train_val=False, **options_)
    print(x['train'])
