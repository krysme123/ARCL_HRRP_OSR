...
# 原代码存在两个关键问题：
# 在denormalize()函数中使用了.mul_()和.add_()这样的inplace操作
# 但输入张量是某个操作的输出视图(view)，PyTorch禁止修改这种视图常
...
def denormalize(tensor):
    ...
    #将经过标准化(Normalization)处理的图像张量还原为原始像素值范围（0到1之间）
    # 确保输入是4D (B,C,H,W)
    if tensor.dim() == 3:
        tensor = tensor.unsqueeze(0)

    denormalized = tensor.clone()  # 创建副本避免修改原tensor
    means = [0.485, 0.456, 0.406]
    stds = [0.229, 0.224, 0.225]

    for b in range(denormalized.size(0)):  # 处理batch维度
        for c in range(3):  # 处理RGB通道
            denormalized[b, c] = denormalized[b, c] * stds[c] + means[c]

    return torch.clamp(denormalized.squeeze(), 0, 1)  # 移除可能的batch维度
...
def find_vgg_layer(arch, target_layer_name):
    ...
    for name, module in arch.named_modules():
        if name == target_layer_name:
            return module
    raise ValueError(f"Layer {target_layer_name} not found in model hierarchy")
